...
def log_file(input, output, metrics, name, node):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "train_op", type=str, help="path to training output extension with txt"
    )
    parser.add_argument(
        "test_op", type=str, help="path to test output extension with txt"
    )
    parser.add_argument(
        "metrics_op", type=str, help="path to metrics output extension with txt"
    )
    parser.add_argument(
        "print_op", type=str, help="path to print output extension with txt"
    )
    parser.add_argument("max_depth", type=int, help="math depth of tree")
    ...
    with open(args.train_ip, "r", newline="") as csvfile:
        ...
        root = build_tree(new_node, args.max_depth, features, data, labels)
        log_file(args.train_ip, args.train_op, args.metrics_op, "train", root)
        log_file(args.test_ip, args.test_op, args.metrics_op, "test", root)

    with open(args.print_op, "w") as writer:
        print_tree(root, data, features, all_label_values, writer, "| ")
