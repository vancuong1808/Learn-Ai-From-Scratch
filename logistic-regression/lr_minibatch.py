...
class LogisticRegression:
    def __init__(self, input_size, num_classes, batch_size, lr=0.01):
    ...
    def fit(self, X, y, epochs=20):
        for epoch in range(epochs):
            ...
            for i in range(0, X_shuffled.shape[0], self.batch_size):
                ...
                yi_onehot = np.zeros((yi.size, self.W.shape[1]))
                yi_onehot[np.arange(yi.size), yi] = 1
...
if __name__ == "__main__":
    ...
    model = LogisticRegression(input_size=X.shape[1], num_classes=len(np.unique(y)), batch_size=16, lr=0.1)
